...
function inject() {
	const css = gulp.src(path.join(conf.paths.src, conf.paths.app, '**/*.css'),
	                     { read: false, base: conf.paths.src });

	const js = gulp.src([path.join(conf.paths.src, conf.paths.app, '*.js'),
		                 path.join(conf.paths.src, conf.paths.app, '*/**/*.js')],
	                    { read: false, base: conf.paths.src });

	const css_app = gulp.src(path.join(conf.paths.src, '*.css'), { read: false });

	const js_app = gulp.src([path.join(conf.paths.src, '*.js')], { read: false });

	gulp.src([path.join(conf.paths.src, '*.js'),
		      path.join(conf.paths.src, '*/**/*.js')], { base: conf.paths.src })
	...
	return gulp.src(path.join(conf.paths.src, conf.paths.mainHtml))
		...
		.pipe($.inject(css_app, { name: 'app', relative: true }))
		.pipe($.inject(js, { relative: true }))
		.pipe($.inject(js_app, { name: 'app', relative: true }))
...
function serveDev() {
	browserSync.init({
		server: {
			...
			middleware: conf.proxy ? proxy('http://lede.lan/ubus', {
				logLevel: 'debug',
...
function watch(done) {
	gulp.watch([path.join(conf.paths.src, conf.paths.mainHtml),
		...
		path.join(conf.paths.src, '**/*.css')], inject);
